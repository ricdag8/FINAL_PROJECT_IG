...
export class LightingManager {
    ...
    setupLighting() {
        ...
        //wall washers create ambient lighting by washing the room walls with colored light they position green spotlights along the perimeter to illuminate wall surface preventing harsh shadows and creating depth in the arcade environment
            for (let i = 0; i < 4; i++) {
    ...
    // creates floor led strips that connect from center point to each machine
    // generates small emissive plane meshes positioned along calculated paths
    createLedPaths() {
        const ledSize = 0.15;
        ...
        //the led path starts at the center point and extends to each machine x coordinate. then it proceeds to reach its z coordinate
        const paths = [
        ...
        paths.forEach(path => {
            const direction = new THREE.Vector3().subVectors(path.to, path.from); //vector from start to end
            const length = direction.length(); //total distance of segment
            direction.normalize();
  ...
  - Room dimensions: 40×8×20 units (width×height×depth)
  - LED size: 0.15×0.15 unit squares
  - Spacing: 0.4 units between LEDs
  - Wave amplitude: 0.6 units (how much LEDs move up/down)
  - Wave frequency: 0.5 (how many waves across the wall)

  Geometry Setup:

  const geoZ = new THREE.PlaneGeometry(ledSize, ledSize); // For front/back walls
  const geoX = new THREE.PlaneGeometry(ledSize, ledSize); // For left/right walls  
  geoX.rotateY(Math.PI / 2); // Rotate 90° to face inward

  Wave Pattern Logic:

  The function creates horizontal bands of LEDs at different heights (Y = 2, 4, 6, 8):

  For Front/Back Walls:
  for (let x = -19 to 19; x += 0.4) { // Across room width
      const yOffset = Math.sin(x * 0.5) * 0.6; // Sine wave modulation
      const currentY = yBase + yOffset; // Apply wave to base height

      // Front wall: Z = -10 + 0.02
      createLed(geoZ, new THREE.Vector3(x, currentY, -9.98));

      // Back wall: Z = 10 - 0.02 (inverted wave)
      createLed(geoZ, new THREE.Vector3(x, yBase - yOffset, 9.98));
  }

  For Left/Right Walls:
  for (let z = -9 to 9; z += 0.4) { // Across room depth
      const yOffset = Math.sin(z * 0.5) * 0.6;
      const currentY = yBase + yOffset;

      // Left wall: X = -20 + 0.02
      createLed(geoX, new THREE.Vector3(-19.98, currentY, z));

      // Right wall: X = 20 - 0.02 (inverted wave)
      createLed(geoX, new THREE.Vector3(19.98, yBase - yOffset, z));
  }

  Visual Result:

  Creates undulating bands of LEDs on walls that:
  - Form sine wave patterns at heights 2, 4, 6, 8 units
  - Have opposite phases on opposing walls (front vs back, left vs right)
  - Change color continuously via the update() function
  - Add dynamic atmosphere to the arcade environment

    
    
    */

    // creates animated led patterns on all four room walls using sine wave modulation positions emissive planes at different heights with wave vertical displacement using sine functions
    createWallLeds() {
